 ...
 */
const readmeTestStrings = String.raw`# Storybook Addon Kit|Click the \*\*Use this template\*\* button to get started.|https://user-images.githubusercontent.com/42671/106809879-35b32000-663a-11eb-9cdc-89f178b5273f.gif`;
  ...
  console.error(
    boxen(
      dedent`
        ...
        ${chalk.red(dedent`
          You are using the default README.md file that comes with the addon kit.
...
const globalPackages = new Set([...globalManagerPackages, ...globalPreviewPackages]);
for (const dependency of peerDependencies) {
  if (globalPackages.has(dependency)) {
    console.error(
      boxen(
        dedent`
          ...
          ${chalk.red(dedent`
            You have a peer dependency on ${chalk.bold(dependency)} which is most likely unnecessary
